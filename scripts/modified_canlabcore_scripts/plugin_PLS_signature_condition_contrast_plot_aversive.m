...
required_fields = {'conditions', 'colors', 'SIG_AVPLS_conditions'};
...
for n = 1:nplots
    ...
    mydata = table2array(DAT.SIG_AVPLS_conditions.(myscaling).(mymetric).(mysignature));
...
% % ---------------------------------------------------------------------
% % List required fields in DAT, in cell array:
% required_fields = {'contrasts', 'contrastnames', 'contrastcolors', 'SIG_AVPLS_contrasts'};
% 
% ok_to_run = plugin_check_required_fields(DAT, required_fields); % Checks and prints warnings
% if ~ok_to_run
%     return
...
%     
%     mysignature = signatures_to_plot{n};
%     mydata = table2array(DAT.SIG_AVPLS_contrasts.(myscaling).(mymetric).(mysignature));
%     
%     % get condition-specific group covariate data if any
%     % this will not work yet, because barplot_columns cannot handle
%     % condition-specific covariates. See "group diffs" script.
