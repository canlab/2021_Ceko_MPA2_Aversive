...
%
%
%        'aversive_lasso': unthresh. bootstrapped weight maps from LASSO-PCR on ratings to 4 stim levels (4 contrast maps per subj) 
%                    Ceko et al (in prep), and the existing pain and visual
%                    canlab patterns: 
%                      Mechpain, Thermpain, NPS, SIIPS, Aversive_sound,
%                      Aversive_visual, PINES 
% &***** TODO replace the above maps w/ not-bootstrapped weight maps (I
% think in lasso stats output_other?) 
%
%         'aversive_pls': unthresholded PLS weight maps on full sample; including general, mech pain, therm pain, aversive sound and aversive visual, 
%          Ceko et al (in prep)
%
%          'pain_pines': NPS, SIIPS, PINES  
% 
%
% :Optional inputs:
%
...
elseif iscell(image_names_or_keyword) || (ischar(image_names_or_keyword) && size(image_names_or_keyword, 1) > 1)
    ...
    switch lower(image_names_or_keyword)
        ...
        case {'aversive_lasso'}
             [image_obj, networknames, imagenames] = load_aversive_sig;
             
        case {'aversive_pls'}
             [image_obj, networknames, imagenames] = load_aversive_pls;
             
        case {'pain_pines'}
             [image_obj, networknames, imagenames] = load_pain_pines;
...
function [image_obj, networknames, imagenames] = load_aversive_lasso

% Load NPS, PINES, Rejection, VPS,
% ------------------------------------------------------------------------

networknames = {'Mechpain' 'Thermpain' 'NPS' 'SIIPS' 'Aversive_sound' 'Aversive_visual' 'PINES' };

imagenames = {'Mechanical_rating_LASSO_PCR_b5000_unthr.nii' ... % Ceko et al. in prep - aversive mechanical
    'Thermal_rating_LASSO_PCR_b5000_unthr.nii' ...              % Ceko et al. in prep - aversive thermal 
    'weights_NSF_grouppred_cvpcr.img' ...                       % Wager et al. 2013 NPS   - somatic pain
    'nonnoc_v11_4_137subjmap_weighted_mean.nii' ...             % Woo 2017 SIIPS - stim-indep pain
    'Sound_rating_LASSO_PCR_b5000_unthr.nii' ...                % Ceko et al. in prep - aversive sound
    'Visual_rating_LASSO_PCR_b5000_unthr.nii' ...               % Ceko et al. in prep - aversive visual 
    'Rating_Weights_LOSO_2.nii'};                               % Chang 2015 PINES - neg emo (aversive visual stimuli)
...
function [image_obj, networknames, imagenames] = load_aversive_pls

% Load PLS sigs
% ------------------------------------------------------------------------

networknames = {'GenS' 'MechS' 'ThermS' 'AudS' 'VisS'};

imagenames = {'General_bplsF_unthr.nii'...% full sample PLS b estimates 
...
function [image_obj, networknames, imagenames] = load_nas
...
imagenames = {'CommonAversive_rating_LASSO_PCR_b5000_unthr.nii'} ... % Ceko et al. in prep - aversive signature
