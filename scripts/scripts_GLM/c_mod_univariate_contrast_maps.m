...
for i = 1:k
    ...
    o2 = removeblobs(o2);
    contrast_t_unc{i} = threshold(contrast_t_fdr{i}, .001, 'unc');
    ...
    if ~isempty(r)
        ...
        figtitle = sprintf('%s_001_unc_regions', DAT.contrastnames{i});
        region_fig_han = activate_figures(o3);
        if ~isempty(region_fig_han)
            set(region_fig_han{1}, 'Tag', figtitle);
            plugin_save_figure;
