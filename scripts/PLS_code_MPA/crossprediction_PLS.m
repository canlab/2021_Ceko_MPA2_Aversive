...
% MC: added last variable description to match N of variables 
...
for m = 1:length(models)
    ...
    eval(['cv_bpls_object_' models{m} ' .dat = apply_mask(dat,gm_mask);']); % mask with GM mask to match 
end
...
for k = 1:5  % for each fold
    ...
    eval(['cv_pls_fold_' num2str(k) ' = cv_bpls;']); % MC: doesn't like num input, has to be str (or char vector)
    ...
    for m = 1:length(models)
       ...
       %eval(['cv_bpls_object_' models{m} '.dat(:, k)cv_ = cv_bpls.dat(:, m);']);
        
       % MC: removed cv_ after .dat(:, k)
...
% The following error occurred converting from single to fmri_data:
% fmri_mask_image.resample_to_image_space: illegal sampleto input.
% 
% Error in crossprediction_PLS (line 171)
